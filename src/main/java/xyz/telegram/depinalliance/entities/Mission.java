...
public class Mission extends BaseEntity {
  ...
  public static List<UserMissionResponse> findByUserId(long userId, boolean isPartner) {
    return find(
      "select m.id, m.groupMission, m.name, m.image, m.description, m.type, m.url, m.point, m.xp, um.status, m.isFake, m.missionRequire, m.box, m.referId, m.partner.id, m.rewardType, m.rewardImage from Mission m left join UserMission um on m.id = um.mission.id and um.user.id =?1 where m.type != ?2 and m.isActive = true" + (
        isPartner ?
  ...
  public static UserMissionResponse findByUserIdAndMissionId(long userId, long missionId) {
    return find(
      "select m.id, m.groupMission, m.name, m.image, m.description, m.type, m.url, m.point, m.xp, um.status, m.isFake, m.missionRequire, m.box, m.referId, m.partner.id, m.rewardType, m.rewardImage from Mission m left join UserMission um on m.id = um.mission.id and um.user.id =?1 where m.id = ?2 and m.isActive = true",
